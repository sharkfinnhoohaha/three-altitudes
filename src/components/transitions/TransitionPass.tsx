 ...
 *
 * Stage 1→2 Noise Wipe (0.20–0.34):
 *   Simplex noise displacement burst at the atmosphere boundary.
 *   Warm color tint during the wipe zone.
 *
...
void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // 1) NOISE WIPE TRANSITION (Stage 1 → 2)
  // ═══════════════════════════════════════════════════════════════════════════
  float wipeZone = smoothstep(0.18, 0.22, uProgress) * (1.0 - smoothstep(0.30, 0.34, uProgress));
  // Bell curve peaked at the boundary (0.25), drives displacement burst
  float wipeBell = exp(-pow((uProgress - 0.25) * 12.0, 2.0));
  float wipeStrength = wipeBell * wipeZone;

  float waveNX = snoise(vec3(uv * 3.8, uTime * 0.45)) * 0.075 * wipeStrength;
  float waveNY = snoise(vec3(uv * 3.8 + 100.0, uTime * 0.38)) * 0.075 * wipeStrength;
  ...
  // Extra chroma burst during wipe
  aberration += wipeStrength * 0.008;
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // 4) WARM TINT DURING WIPE ZONE
  // ═══════════════════════════════════════════════════════════════════════════
  vec3 effected = vec3(r, g, b);
  float wipeTint = wipeStrength * 0.35;
  effected = mix(effected, effected * vec3(1.12, 0.92, 0.72), wipeTint);

  // ═══════════════════════════════════════════════════════════════════════════
  // 5) HORIZON CLEAN PASS
  // ═══════════════════════════════════════════════════════════════════════════
